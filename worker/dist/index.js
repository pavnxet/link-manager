...
async function checkAuthentication(supabase, telegramUserId) {
  const { data, error } = await supabase.from("bot_sessions").select("is_authenticated").eq("telegram_user_id", telegramUserId).single();
  if (error || !data) {
    return false;
  }
...
async function getUserInfo(supabase, telegramUserId) {
  const { data, error } = await supabase.from("bot_sessions").select("is_authenticated").eq("telegram_user_id", telegramUserId).single();
  if (error || !data || !data.is_authenticated) {
    return { userId: null, isAuthenticated: false };
  }
  const { data: adminData } = await supabase.from("bot_users").select("id").limit(1).single();
  return {
...
async function authenticateUser(supabase, telegramUserId, username, adminUsername, adminPassword, envAdminUsername, envAdminPassword) {
  const credsMatch = adminUsername.length === envAdminUsername.length && adminPassword.length === envAdminPassword.length && constantTimeCompare(adminUsername, envAdminUsername) && constantTimeCompare(adminPassword, envAdminPassword);
  if (!credsMatch) {
    return { success: false, message: "\u274C Invalid credentials!" };
  }
  const { data: existingSession } = await supabase.from("bot_sessions").select("*").eq("telegram_user_id", telegramUserId).single();
  if (existingSession) {
    const { error } = await supabase.from("bot_sessions").update({
...
async function saveLink(supabase, url, title, pageTitle, category, telegramUserId) {
  try {
    const userInfo = await getUserInfo(supabase, telegramUserId);
    if (!userInfo.isAuthenticated || !userInfo.userId) {
      return { success: false, error: "User not authenticated" };
    }
    const { data: existingLink } = await supabase.from("links").select("id").eq("url", url).eq("user_id", userInfo.userId).single();
    if (existingLink) {
...
__name(saveLink, "saveLink");
async function deleteLink(supabase, displayNumber, telegramUserId) {
  try {
    const userInfo = await getUserInfo(supabase, telegramUserId);
    if (!userInfo.isAuthenticated || !userInfo.userId) {
      return { success: false, error: "User not authenticated" };
...
async function getLinks(supabase, limit = 20, offset = 0, searchQuery, telegramUserId) {
  try {
...
async function getLinkByDisplayNumber(supabase, displayNumber, telegramUserId) {
  try {
    const userInfo = await getUserInfo(supabase, telegramUserId);
    if (!userInfo.isAuthenticated || !userInfo.userId) {
...
__name(getLinkByDisplayNumber, "getLinkByDisplayNumber");
async function getAllLinksForBackup(supabase, telegramUserId) {
  try {
    const userInfo = await getUserInfo(supabase, telegramUserId);
    if (!userInfo.isAuthenticated || !userInfo.userId) {
      return { success: false, error: "Not authenticated" };
...
__name(getAllLinksForBackup, "getAllLinksForBackup");
async function checkRateLimit(supabase, telegramUserId) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const oneMinuteAgo = new Date(Date.now() - 6e4).toISOString();
  try {
    supabase.rpc("cleanup_old_rate_limits").then(() => {
    });
    const { data } = await supabase.from("rate_limits").select("count").eq("telegram_user_id", telegramUserId).gte("window_start", oneMinuteAgo).single();
    if (!data) {
...
async function getStats(supabase, telegramUserId) {
  try {
    const userInfo = await getUserInfo(supabase, telegramUserId);
    if (!userInfo.isAuthenticated || !userInfo.userId) {
      return { totalLinks: 0, totalUsers: 0 };
    }
    const { count: linkCount } = await supabase.from("links").select("*", { count: "exact", head: true }).eq("user_id", userInfo.userId);
    const { count: userCount } = await supabase.from("bot_sessions").select("*", { count: "exact", head: true }).eq("is_authenticated", true);
    return {
...
function createSaveInlineKeyboard(url, pageTitle, title) {
  return {
    inline_keyboard: [
      [
...
__name(createSaveInlineKeyboard, "createSaveInlineKeyboard");
...
__name(formatLinkMessage, "formatLinkMessage");
...
function formatHelpMessage() {
  return `\u{1F916} <b>Link Vault Bot Commands</b>

\u{1F510} <b>/login &lt;username&gt; &lt;password&gt;</b> - Authenticate as admin

\u{1F4E5} <b>Send any link</b> - Bot will ask to save it

\u{1F4CB} <b>/list [query]</b> - List saved links (optional search)

\u{1F5D1}\uFE0F <b>/delete &lt;number&gt;</b> - Delete link by number

\u{1F4BE} <b>/backup</b> - Export all links as JSON

\u{1F4CA} <b>/stats</b> - Show statistics

\u2753 <b>/help</b> - Show this help message

\u{1F4A1} <b>Tip:</b> Just send me any URL and I'll ask if you want to save it!`;
}
__name(formatHelpMessage, "formatHelpMessage");
function formatStatsMessage(totalLinks, totalUsers) {
  return `\u{1F4CA} <b>Link Vault Statistics</b>

\u{1F517} Total Links: <b>${totalLinks}</b>
\u{1F465} Total Users: <b>${totalUsers}</b>

Keep building your vault! \u{1F680}`;
}
__name(formatStatsMessage, "formatStatsMessage");
...
\u{1F517} <a href="${escapeHtml(url)}">${escapeHtml(url)}</a>`,
      ...
      if (data.startsWith("delete:")) {
        ...
        if (result.success) {
          await answerCallbackQuery(env.TELEGRAM_BOT_TOKEN, callback.id, `\u{1F5D1}\uFE0F Deleted #${linkId}`);
          await sendMessage(env.TELEGRAM_BOT_TOKEN, chatId, `\u{1F5D1}\uFE0F Link #${linkId} has been deleted.`);
        } else {
      ...
      if (data.startsWith("list:")) {
        ...
        const result = await getLinks(supabase, 20, offset, void 0, userId);
        if (result.success && result.links) {
          const hasMore = result.links.length === 20;
          let message = "\u{1F4CB} <b>Your Links</b>\n\n";
...
`;
          ...
          const replyMarkup = offset > 0 || hasMore ? { inline_keyboard: [
            [
    ...
    if (update.message) {
      ...
      if (urls.length > 0 && !text.startsWith("/")) {
        ...
        await sendMessage(
          env.TELEGRAM_BOT_TOKEN,
          chatId,
          `\u{1F517} <b>Found a link!</b>

\u{1F4C4} <b>Title:</b> ${escapeHtml(defaultTitle)}
\u{1F310} <b>Page:</b> ${escapeHtml(pageTitle)}
\u{1F517} <b>URL:</b> <a href="${escapeHtml(url)}">${escapeHtml(url)}</a>
        ...
        switch (command) {
          ...
          case "/list":
            ...
            if (listResult.success && listResult.links) {
              let message2 = searchQuery ? `\u{1F50D} <b>Search Results for "${escapeHtml(searchQuery)}"</b>

` : `\u{1F4CB} <b>Your Latest Links</b>
...
`;
              ...
              const replyMarkup = hasMore ? { inline_keyboard: [[{ text: "Next \u27A1\uFE0F", callback_data: "list:20" }]] } : void 0;
              await sendMessage(env.TELEGRAM_BOT_TOKEN, chatId, message2, replyMarkup);
            } else {
          ...
          case "/delete":
            ...
            if (linkToDelete.success && linkToDelete.link) {
              const confirmMarkup = {
                inline_keyboard: [[
                  { text: "\u{1F5D1}\uFE0F Yes, Delete", callback_data: `delete:${displayNum}` },
                  { text: "\u274C Cancel", callback_data: "cancel" }
                ]]
              };
              await sendMessage(
...
${formatLinkMessage(linkToDelete.link)}`,
          ...
          case "/backup":
            ...
            if (backupResult.success && backupResult.links) {
              ...
              const fileName = `link-vault-backup-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.json`;
              const formData = new FormData();
              formData.append("chat_id", chatId.toString());
              formData.append("document", new Blob([jsonData], { type: "application/json" }), fileName);
              formData.append("caption", `\u{1F4E6} Backup of ${backupResult.links.length} links`);
